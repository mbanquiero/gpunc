...
static __device__ __inline__ unsigned int RND(unsigned int& seed)
{
    /*unsigned int x = random_data[seed % RANDOM_DATA_SIZE];
    seed += x;
    return x;
    */
    return lcg(seed);
...
__global__ void filterInKernel(char* data_out, char* data, const int step, const int DIMW, const int NUMPROC)
{
    ...
    for (int q = 0; q < max_Q; ++q)
    {
        ...
        if (N == NUMPROC - 1)
        {
            n = RND(seed) % (NUMPROC - 1);
        }
        else
        {
            n = RND(seed) % (NUMPROC - 2);
            if (n >= N)
                n++;
        }
        int j = (n * DIMP + RND(seed) % DIMP) * DIMW;
        if (data[j] > 0)
        {
            // si pasa el filtro lo dejo entrar
            if (passInFilter(data + j, N, DIMW, NUMPROC,n))
            {
                for (int t = 0; t < DIMW; ++t)
                {
